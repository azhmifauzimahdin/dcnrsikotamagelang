...
@section('container')
    <x-title.title>Klaim BPJS</x-title.title>
    <x-alert.secondary fontWeight="normal">
        Pilih file berisi data klaim dalam format xlsx, xls, ata csv. Pastikan header tabel berada di
        baris pertama supaya data dapat terbaca oleh sistem. Untuk lebih jelas bisa lihat di <a
            href="{{ route('claim.download') }}" class="text-blue-500">template</a> data klaim. Jika terjadi
    ...
    <script>
        ...
        function cekHeader(jsonData) {
            ...
            if (jsonData.length > 0) {
                ...
                let totalTarif = header.includes('Total Tarif');
                let tarifRS = header.includes('Tarif RS');
                ...
                appendColumn(totalTarif, '[Total Tarif]');
                appendColumn(tarifRS, '[Tarif RS]');
                ...
                if (tglMasuk && tglPulang && noRM && namaPasien && sep && totalTarif && tarifRS) {
            ...
            if (jsonData.length > 0) {
                ...
                jsonData.forEach((row, i) => {
                    htmlData += '<tr class="odd:bg-gray-50 even:bg-gray-100"><td>' + (i + 1) + '</td><td>' + row[
                        ...
                        '</td><td>' + row['No. Klaim / SEP'] + '</td><td>' + row['Total Tarif'] + '</td><td>' + row[
                            'Tarif RS'] + '</td></tr>';
                });
